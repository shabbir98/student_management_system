...
async function addStudent() {
  ...
  console.log(
    `Student ${student.name} added with ID ${
      student.id
    } with current balance $${student.viewBalance()}`
  );
}
...
async function viewBalance() {
  ...
  if (studentWithBalance) console.log(`$${studentWithBalance.viewBalance()}`);
}
...
async function payTuition() {
  ...
  if (studentToPay) {
    const tuitionAmount = parseFloat(
      await getInput('Enter the amount to pay for tuition:')
    );

    studentToPay.payTuition(tuitionAmount);

    console.log(
      `${studentToPay.name} paid $${tuitionAmount} for tuition fees.`
    );
  }
}
...
async function studentStatus() {
  ...
  if (showStatus) showStatus.showStatus();
}

async function updateBalance() {
  const updateAmountForStudent = await selectStudent();
  if (updateAmountForStudent) {
    const updateAmount = parseFloat(
      await getInput('Enter the amount to be added to the current balance:')
    );
